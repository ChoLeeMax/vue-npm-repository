    ...
    <div class="step-list">
      <div class="step-item" v-for="(item, index) in stepData" :key="index">
        <div
          class="setpNorm"
          :class="{
            stepActive: item.status == '1',
            stepNone: item.status == '0'
          }"
        >
          <div class="step-back">
 ...
 * @description  处置流程组件
 * @update 2019.9.3
 * @update 2019.11.21 rogers 更新了左侧的宽度值设置为80px 
...
export default {
  ...
  methods: {
    ...
    /**
     * 获取步骤节点最大宽度，并统一设置，解决标题长度导致右侧内容不能左对齐问题
     * @param {Array} doms 步骤描述dom节点集合
     * */
    getMaxWidth(doms) {
      var maxWidth = 0;
  ...
  mounted() {
    ...
    let doms = document.getElementsByClassName("step-title");
    this.getMaxWidth(doms);
  }
};
</script>
